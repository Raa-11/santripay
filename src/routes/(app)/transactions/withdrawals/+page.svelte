<script lang="ts">
	...
	// Can only submit when: program selected, amount > 0, amount ≤ balance, not submitting
	...
	async function loadEnrollments(studentId: string) {
		...
		try {
			...
			// Only show enrollments with a positive balance for withdrawals
	...
	function selectEnrollment(e: Enrollment) {
		...
		programOpen = false;
		// Reset amount if it would overdraw the new selection
	...
	<div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5 items-stretch">
		...
		<div class="rounded-xl border bg-card p-5 flex flex-col gap-4">
			...
			<div class="rounded-lg px-4 py-3 space-y-0.5 {isOverdrawn ? 'bg-destructive/10 border border-destructive/20' : 'bg-muted/50'}">
				<p class="text-[10px] font-bold uppercase tracking-widest {isOverdrawn ? 'text-destructive' : 'text-muted-foreground'}">
					Balance After
				</p>
