<script lang="ts">
    ...
    const canSubmit = $derived(
        !!selectedEnrollment && amount > 0 && !isOverdrawn && !submitting,
    );

    // ── Derived (bulk) ────────────────────────────────────
    const selectedPlan = $derived(
        data.plans.find((p) => p.id === bulkPlanId) ?? null,
    );
    const totalPlanBalance = $derived(
        bulkMembers.reduce((s, m) => s + m.currentAmount, 0),
    );
    ...
    function computeDistribution(total: number, members: Member[]): DistRow[] {
        ...
        return members.map((m, i) => {
            ...
            return {
                ...
                capped,
                balanceAfter: balance.minus(share).toNumber(),
            };
        });
    }
    ...
    const bulkExceedsBalance = $derived(
        totalPlanBalance > 0 && bulkAmount > totalPlanBalance,
    );
    ...
    const fmt = (n: number) =>
        "Rp " + new Intl.NumberFormat("id-ID").format(Math.round(Math.abs(n)));
    ...
    async function loadRecentTx(studentId: string) {
        const res = await fetch(
            `/api/students/${studentId}/recent-deposits?type=WITHDRAW`,
        );
            ...
            if (result.type === "success") {
                ...
                if (selectedStudent)
                    await Promise.all([
                        ...
                        loadRecentTx(selectedStudent.id),
                    ]);
                setTimeout(() => {
                    withdrawSuccess = false;
                }, 4000);
                ...
                withdrawError = String(
                    result.data?.error ?? "Gagal memproses penarikan",
                );
            }
        };
    };
    ...
    async function selectBulkPlan(id: string) {
        ...
        try {
            ...
            bulkMembers = raw.map((r: { savingsId: string; studentName: string; studentNis: string; studentClass: string | null; currentAmount: number }) => ({
                id: r.savingsId,
                name: r.studentName,
                nis: r.studentNis,
        ...
        return async ({
            ...
            if (result.type === "success") {
                ...
                if (bulkPlanId) {
                    ...
                    try {
                        const res = await fetch(
                            `/api/saving-plans/${bulkPlanId}/members`,
                        );
                        const raw = await res.json();
                        bulkMembers = raw.map((r: { savingsId: string; studentName: string; studentNis: string; studentClass: string | null; currentAmount: number }) => ({
                            id: r.savingsId,
                ...
                bulkError = String(
                    result.data?.error ?? "Gagal memproses penarikan massal",
                );
            }
        };
    };
...
<div class="flex flex-col gap-6 flex-1">
    ...
    <div class="flex items-start justify-between">
        <div>
            ...
            <p class="text-sm text-muted-foreground mt-1">
                ...
                <span class="text-primary font-medium">surplus SPP</span>, atau
                pencairan tujuan.
            </p>
        </div>
        <Button
    ...
    <div class="flex gap-0.5 rounded-lg border bg-muted/30 p-0.5 w-fit">
        {#each [{ key: "per-siswa", label: "Per Siswa" }, { key: "massal", label: "Per Program Tabungan" }] as tab}
            <button
                onclick={() => (activeTab = tab.key as "per-siswa" | "massal")}
                class="rounded px-4 py-1.5 text-xs font-semibold transition-colors {activeTab ===
                tab.key
    ...
    {#if activeTab === "per-siswa"}
        <div
            class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5 items-stretch"
        >
            <!-- Form card -->
            <form
                method="POST"
            ...
            >
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    {#if withdrawSuccess}
                        <span
                            class="flex items-center gap-1 text-[10px] font-medium text-emerald-600"
                        >
                            <HugeiconsIcon icon={Tick02Icon} size={11} /> tersimpan
                            · {lastRefNo}
                        </span>
                    {:else}
                ...
                <Field.Field class="space-y-1.5">
                    ...
                    <Field.Content>
                        {#if selectedStudent}
                            <div
                                class="flex items-center gap-3 rounded-lg border bg-muted/40 px-3 py-2.5"
                            >
                                <span
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    >
                                        {selectedStudent.nis}{selectedStudent.class
                        ...
                        {:else}
                            <div class="relative">
                                <div
                                    class="flex items-center rounded-lg border bg-muted/40 px-3 gap-2 focus-within:ring-2 focus-within:ring-ring"
                                >
                                    <HugeiconsIcon
                                        icon={Search01Icon}
                                        size={14}
                                ...
                                {#if searchOpen && searchResults.length > 0}
                                    <div
                                        class="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border bg-card shadow-lg overflow-hidden"
                                    >
                                        {#each searchResults as s (s.id)}
                                            ...
                                            >
                                                <span
                                                    class="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-[10px] font-bold text-primary"
                                                >
                                                    {getInitials(s.name)}
                                                </span>
                                                <div class="flex-1 min-w-0">
                ...
                <Field.Field class="space-y-1.5">
                    <Field.Label
                        class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground"
                        >Program Tabungan</Field.Label
                    >
                    <Field.Content>
                        {#if !selectedStudent}
                            <div
                                class="flex items-center gap-3 rounded-lg border bg-muted/20 px-3 py-2.5 opacity-50"
                            >
                                <span
                        ...
                        {:else if enrollments.length === 0}
                            <div
                                class="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5"
                            >
                                <span class="text-amber-600 text-sm">⚠</span>
                                <p class="text-sm text-amber-700">
                                    Tidak ada program dengan saldo tersedia
                                    untuk ditarik.
                                </p>
                            </div>
                        {:else}
                            <div class="relative">
                                ...
                                >
                                    <span
                                        class="flex size-8 shrink-0 items-center justify-center rounded-lg {selectedEnrollment
                                            ? 'bg-primary/10'
                                            : 'bg-muted'}"
                                    >
                                        <HugeiconsIcon
                                    ...
                                    {#if selectedEnrollment}
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p
                                                class="text-[11px] text-muted-foreground"
                                            >
                                                {selectedEnrollment.planCode} · Saldo:
                                                {fmt(
                                                    selectedEnrollment.currentAmount,
                                                )}
                                            </p>
                                        </div>
                                    {:else}
                                    ...
                                    >
                                        {#each enrollments as e (e.id)}
                                            <button
                                                ...
                                                onclick={() =>
                                                    selectEnrollment(e)}
                                                class="w-full flex items-center justify-between px-3 py-2.5 hover:bg-muted/50 transition-colors text-left {e.id ===
                                            ...
                                            >
                                                ...
                                                <span
                                                    class="text-xs font-semibold text-emerald-700 shrink-0 ml-3"
                                                    >{fmt(
                                                        e.currentAmount,
                                                    )}</span
                                                >
                    ...
                    <Field.Content>
                        <div class="relative">
                            ...
                            <Input
                                ...
                                bind:value={amount}
                                min={0}
                                max={selectedEnrollment?.currentAmount ??
                                    undefined}
                                disabled={!selectedEnrollment}
                                class="pl-10 pr-14 h-12 text-xl font-bold bg-muted/40 border disabled:opacity-40 {isOverdrawn
                        ...
                        {#if isOverdrawn && amount > 0}
                            <p
                                class="text-[11px] text-destructive font-medium mt-1"
                            >
                                Melebihi saldo tersedia sebesar {fmt(
                    ...
                    <div class="flex flex-wrap gap-1.5 pt-1">
                        {#each presets as p}
                            <button
                                ...
                                disabled={!selectedEnrollment ||
                                    p > balanceBefore}
                                onclick={() => (amount = p)}
                                class="text-[11px] font-semibold px-2.5 py-1 rounded-full border transition-all disabled:opacity-30 {amount ===
                                p
                        ...
                        {#if selectedEnrollment && balanceBefore > 0}
                            <button
                                ...
                                class="text-[11px] font-bold px-3 py-1 rounded-full border transition-all {amount ===
                                balanceBefore
                                    ? 'bg-destructive text-destructive-foreground border-destructive'
                                    : 'bg-amber-100 hover:bg-amber-200 text-amber-800 border-transparent'}"
                            >
                                Maks ({new Intl.NumberFormat("id-ID").format(
                                    balanceBefore,
                                )})
                            </button>
                        {/if}
                ...
                <div class="flex items-center justify-between">
                    <p class="text-[11px] text-muted-foreground">
                        Dicatat ke <span class="font-medium text-foreground"
                            >entri jurnal</span
                        > · jejak audit aktif
                    </p>
                    <div class="flex items-center gap-2">
                        <Button
                            ...
                            size="sm"
                            class="text-xs h-9"
                            onclick={reset}>Atur Ulang</Button
                        >
                        <Button
                            type="submit"
                            size="sm"
                            variant="destructive"
                            class="text-xs h-9 gap-1.5 font-semibold"
                            disabled={!canSubmit}
                        >
                            {#if submitting}
                                <span
            ...
            <div class="rounded-xl border bg-card p-5 flex flex-col gap-4">
                ...
                <div class="space-y-0.5">
                    ...
                    <p class="text-xl font-bold text-destructive">
                        − {fmt(amount > 0 ? amount : 0)}
                    </p>
                </div>
                <div
                    class="rounded-lg px-4 py-3 space-y-0.5 {isOverdrawn
                        ? 'bg-destructive/10 border border-destructive/20'
                        : 'bg-muted/50'}"
                >
                    <p
                        class="text-[10px] font-bold uppercase tracking-widest {isOverdrawn
                            ? 'text-destructive'
                            : 'text-muted-foreground'}"
                    >
                        Saldo Setelah
                    ...
                    <p
                        class="text-2xl font-bold tracking-tight {isOverdrawn
                            ? 'text-destructive'
                            : ''}"
                    >
                ...
                <div class="space-y-2.5">
                    {#each [{ label: "Program", value: selectedEnrollment?.planCode ?? "—" }, { label: "Tanggal", value: dateLabel }, { label: "Operator", value: data.user?.name ?? "—" }] as row}
                        <div class="flex items-center justify-between">
                            <span
                                class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground"
                                >{row.label}</span
                ...
                {:else}
                    <div>
                        {#each recentTx as tx (tx.id)}
                            <div
                                class="flex items-center justify-between py-2.5 {tx !==
                                recentTx[recentTx.length - 1]
                            ...
                            >
                                ...
                                >
                                    <span
                                        class="text-[11px] text-muted-foreground tabular-nums shrink-0 w-[110px]"
                                        >{txDateTime(tx.createdAt)}</span
                                    >
                                    <span
                                        class="text-[10px] font-mono font-bold text-muted-foreground bg-muted px-1.5 py-0.5 rounded shrink-0"
                                        >{tx.planCode}</span
                                    >
                                    <span
                                        class="text-xs text-foreground font-semibold truncate max-w-[200px] md:max-w-[300px]"
                                        >{tx.planName}</span
                                    >
                                    {#if tx.isReversed}
                                        <span
                                            class="text-[10px] text-amber-600 font-medium shrink-0"
                                            >dibatalkan</span
                                        >
                                    {/if}
                                </div>
                                <span
                                    class="text-xs font-bold tabular-nums {tx.isReversed
    ...
    {#if activeTab === "massal"}
        ...
        >
            ...
            <form
                ...
                onsubmit={(e) => {
                    if (!canBulkSubmit) e.preventDefault();
                }}
                class="rounded-xl border bg-card p-5 flex flex-col gap-5"
            >
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    {#if bulkSuccess}
                        ...
                        >
                            ...
                            {bulkLastCount} siswa · {fmt(Number(bulkLastTotal))} ·
                            {bulkLastRef}
                        </span>
                    {/if}
                </div>
                    ...
                    <Field.Content>
                        <div class="relative">
                            <button
                                type="button"
                                onclick={() => {
                                    bulkPlanOpen = !bulkPlanOpen;
                                }}
                                class="w-full flex items-center gap-3 rounded-lg border bg-muted/40 px-3 py-2.5 text-left hover:bg-muted/60 transition-colors"
                            >
                                <span
                                    class="flex size-8 shrink-0 items-center justify-center rounded-lg {selectedPlan
                                        ? 'bg-primary/10'
                                        : 'bg-muted'}"
                                >
                                    <HugeiconsIcon
                            ...
                            {#if bulkPlanOpen}
                                <div
                                    class="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border bg-card shadow-lg overflow-hidden max-h-56 overflow-y-auto"
                                >
                                    {#each data.plans as p (p.id)}
                                        <button
                                            type="button"
                                            onclick={() => selectBulkPlan(p.id)}
                                            class="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted/50 transition-colors text-left {p.id ===
                                            bulkPlanId
                                                ? 'bg-muted/30'
                                                : ''}"
                                        >
                    ...
                    <Field.Content>
                        <div class="relative">
                            ...
                            <Input
                                ...
                                min={0}
                                max={totalPlanBalance > 0
                                    ? totalPlanBalance
                                    : undefined}
                                step={1}
                                disabled={!bulkPlanId || bulkMembersLoading}
                                class="w-full pl-10 pr-14 h-12 text-xl font-bold bg-muted/40 border disabled:opacity-40 {bulkExceedsBalance
                                    ? 'border-destructive focus-visible:ring-destructive'
                                    : ''}"
                            />
                            <span
                        ...
                        {#if bulkPlanId && !bulkMembersLoading}
                            <p class="text-[11px] text-muted-foreground mt-1">
                                Total saldo program: <span
                                    class="font-semibold text-foreground"
                                    >{fmt(totalPlanBalance)}</span
                                >
                        ...
                        {#if bulkAmount > totalPlanBalance && totalPlanBalance > 0}
                            <p
                                class="text-[11px] text-destructive font-medium mt-1"
                            >
                                Melebihi total saldo program. Jumlah akan
                        ...
                        <div class="flex flex-wrap gap-1.5 pt-1">
                            {#each presets as p}
                                <button
                                    ...
                                    class="text-[11px] font-semibold px-2.5 py-1 rounded-full border transition-all disabled:opacity-30 {bulkAmount ===
                                    p
                                        ? 'bg-destructive text-destructive-foreground border-destructive'
                                        : 'bg-muted/50 text-muted-foreground hover:bg-muted border-transparent'}"
                                >
                            ...
                            <table class="w-full text-xs">
                                <thead>
                                    <tr class="bg-muted/50 border-b">
                                        ...
                                        >
                                        <th
                                            class="px-3 py-2 text-right font-semibold text-muted-foreground"
                                            >Saldo</th
                                        >
                                        <th
                                            class="px-3 py-2 text-right font-semibold text-muted-foreground"
                                    ...
                                    {#each distribution as row (row.id)}
                                        ...
                                        >
                                            ...
                                            <td
                                                ...
                                                >{fmt(row.currentAmount)}</td
                                            >
                                            <td
                                                class="px-3 py-2 text-right tabular-nums font-semibold text-destructive"
                                            >
                                                {fmt(row.share)}
                                                {#if row.capped}
                                                    <span
                                                        class="text-[9px] text-amber-600 ml-1"
                                                        >maks</span
                                                    >
                                                {/if}
                                            </td>
                                            <td
                                                class="px-3 py-2 text-right tabular-nums {row.balanceAfter ===
                                                0
                                                    ? 'text-muted-foreground'
                                                    : ''}"
                                            >
                                ...
                                <tfoot>
                                    <tr
                                        class="bg-muted/30 border-t font-semibold"
                                    >
                                        <td
                                            class="px-3 py-2 text-muted-foreground"
                                            >{distribution.length} siswa</td
                                        >
                                        <td
                                            class="px-3 py-2 text-right tabular-nums text-muted-foreground"
                                            >{fmt(totalPlanBalance)}</td
                                        >
                                        <td
                                            class="px-3 py-2 text-right tabular-nums text-destructive"
                                        >
                                            {fmt(actualTotal)}
                ...
                {:else if bulkPlanId && bulkMembers.length === 0 && !bulkMembersLoading}
                    <div
                        class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-700"
                    >
                ...
                <div class="flex items-center justify-between">
                    <p class="text-[11px] text-muted-foreground">
                        Setiap siswa dicatat sebagai <span
                    ...
                    <div class="flex items-center gap-2">
                        <Button
                            ...
                            size="sm"
                            class="text-xs h-9"
                            onclick={resetBulk}>Atur Ulang</Button
                        >
                        <Button
                            type="submit"
                            size="sm"
            ...
            <div class="rounded-xl border bg-card p-5 flex flex-col gap-4">
                ...
                <div class="space-y-0.5">
                    ...
                    {:else}
                        <p class="text-xl font-bold text-muted-foreground/40">
                            —
                        </p>
                    {/if}
                </div>
                ...
                {#if bulkAmount > 0 && actualTotal !== bulkAmount && bulkMembers.length > 0}
                    <div
                        class="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5 space-y-0.5"
                    >
                        <p
                            class="text-[10px] font-bold uppercase tracking-widest text-amber-700"
                        >
                            Aktual Ditarik
                        </p>
                ...
                <div class="space-y-2.5">
                    {#each [{ label: "Program", value: selectedPlan?.code ?? "—" }, { label: "Tanggal", value: dateLabel }, { label: "Operator", value: data.user?.name ?? "—" }] as row}
                        <div class="flex items-center justify-between">
                            <span
                                class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground"
                                >{row.label}</span
